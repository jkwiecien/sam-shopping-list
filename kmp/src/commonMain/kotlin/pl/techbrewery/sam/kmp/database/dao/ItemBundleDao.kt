...
interface ItemBundleDao {
    ...
    fun getRecipesWithItems(): Flow<List<ItemBundleWithItems>>

    // Get all item bundles with their single items, filtered by a query string
    @Transaction
    @Query("SELECT * FROM item_bundles WHERE name LIKE :query || '%'")
    fun getRecipesWithItemsFlow(query: String): Flow<List<ItemBundleWithItems>>

    @Transaction
    @Query("SELECT * FROM item_bundles WHERE name LIKE :recipeName ")
    fun getRecipeWithItemsByName(recipeName: String): ItemBundleWithItems?
